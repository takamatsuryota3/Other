...
- PHPの勉強
    - サンプル実装
##### 所感  
- PHPのサンプル実装で、以前作った事がある、カレンダーを作り始めたが、結構忘れている箇所があったので、
　サンプル実装を通して、しっかりと知識の定着をさせたい
- PHPだけでなく、JavaScriptで動きをつけるような事もやってみたい
- カレンダーの装飾は、cssファイルで作成するようにしているが、機会があれば、BootStrapも使っていきたい
- AWSやJavaの勉強は一旦保留で、先ずはサンプル実装を終わらせたい
##### 参考になったページ  
- [【動画・サンプルコード】PHPカレンダーの作り方](https://codeforfun.jp/php-calendar/)  
- [PHPでカレンダーを作成【サンプルコード有】](https://kinocolog.com/php_calendar/)  
- [PHP+HTMLで作る 祝日対応のカレンダーの作り方| Start Point](https://www.start-point.net/blog/web/php/calendar/)  
->サンプル実装の時に参考にしたもの
---
##### 日付  
- 2021/06/13
...
- AWSの勉強
    - IAM
        ...
        - IAMグループへのポリシー適用
##### 所感  
- 火曜日から土曜日、いろいろとバタバタしていて、
　自己研鑽を積む事が出来なかったので反省
- 業務でPHPに触れていたが、自宅PC環境ではやっていなかったので、環境構築と軽くサンプル実装をしたのだが、
一部忘れている箇所があったので、しっかりと復習して定着させたい
##### 参考になったページ  
- [【PHP超入門コース】03.環境構築（for Windows）｜プログラミングをする準備をしよう！【プログラミング初心者向け入門講座】](https://kino-code.com/course-php03-environment-for-windows/)  
->環境構築する際に役に立った
- [PHPで数値をチェックする4つの方法を比較](https://www.flatflag.nir87.com/is_numeric-682#is_intint)  
->サンプル実装の中で、数値のチェックする処理を実装する際に役に立った
